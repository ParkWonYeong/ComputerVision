...
good = []
for m, n in matches:
    if m.distance < 0.3*n.distance:     # best match에 대한 ratio distance의 임계값인 듯하다.
        good.append([m])

img3 = cv2.drawMatchesKnn(img1, kp1, img2, kp2, good, None, flags=26)    # flags는 Blob이 보이는 채널??같은것같다. 2의 n제곱 값이 할당되지 않으면 에러 발생.
